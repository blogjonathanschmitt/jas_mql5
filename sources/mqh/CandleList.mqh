...
class CCandleList
  {
protected:
   CCandle          *m_first_node;       // pointer to the first element of the list
   CCandle          *m_last_node;        // pointer to the last element of the list
   CCandle          *m_curr_node;        // pointer to the current element of the list
...
public:
   ...
   virtual CCandle  *CreateElement(void) { return(NULL); }
   //--- methods of filling the list
   int               Add(CCandle *new_node);
   int               Insert(CCandle *new_node,int index);
   //--- methods for navigating
   int               IndexOf(CCandle *node);
   CCandle          *GetNodeAtIndex(int index);
   CCandle          *GetFirstNode(void);
   CCandle          *GetPrevNode(void);
   CCandle          *GetCurrentNode(void);
   CCandle          *GetNextNode(void);
   CCandle          *GetLastNode(void);
   //--- methods for deleting
   CCandle          *DetachCurrent(void);
   bool              DeleteCurrent(void);
   bool              Delete(int index);
   void              Clear(void);
   //--- method for comparing lists
   bool              CompareList(CCandleList *List);
   //--- methods for changing
   void              Sort(int mode);
   bool              MoveToIndex(int index);
   bool              Exchange(CCandle *node1,CCandle *node2);
   //--- method for searching
   CCandle          *Search(CCandle *element);
protected:
   void              QuickSort(int beg,int end,int mode);
   CCandle          *QuickSearch(CCandle *element);
  };
//+------------------------------------------------------------------+
//| Constructor                                                      |
//+------------------------------------------------------------------+
CCandleList::CCandleList(void) : m_first_node(NULL),
                                 m_last_node(NULL),
                                 m_curr_node(NULL),
                                 m_curr_idx(-1),
                                 m_data_total(0),
...
//| Method QuickSort                                                 |
//+------------------------------------------------------------------+
void CCandleList::QuickSort(int beg,int end,int mode)
  {
   int      i,j,k;
   CCandle *i_ptr,*j_ptr,*k_ptr;
//---
...
//+------------------------------------------------------------------+
int CCandleList::IndexOf(CCandle *node)
  {
//--- check
...
//+------------------------------------------------------------------+
int CCandleList::Add(CCandle *new_node)
  {
//--- check
...
//+------------------------------------------------------------------+
int CCandleList::Insert(CCandle *new_node,int index)
  {
   CCandle *tmp_node;
//--- check
   if(!CheckPointer(new_node))
...
//+------------------------------------------------------------------+
CCandle *CCandleList::GetNodeAtIndex(int index)
  {
   int      i;
   bool     revers;
   CCandle *result;
...
//+------------------------------------------------------------------+
CCandle *CCandleList::GetFirstNode(void)
  {
//--- check
...
//+------------------------------------------------------------------+
CCandle *CCandleList::GetPrevNode(void)
  {
//--- check
...
//+------------------------------------------------------------------+
CCandle *CCandleList::GetCurrentNode(void)
  {
   return(m_curr_node);
  }
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
CCandle *CCandleList::GetNextNode(void)
  {
//--- check
...
//+------------------------------------------------------------------+
CCandle *CCandleList::GetLastNode(void)
  {
//--- check
...
//+------------------------------------------------------------------+
CCandle *CCandleList::DetachCurrent(void)
  {
   CCandle *tmp_node,*result=NULL;
//--- check
...
//+------------------------------------------------------------------+
bool CCandleList::DeleteCurrent(void)
  {
...
//+------------------------------------------------------------------+
bool CCandleList::Delete(int index)
  {
   if(GetNodeAtIndex(index)==NULL)
      return(false);
//--- result
...
//+------------------------------------------------------------------+
bool CCandleList::CompareList(CCandleList *List)
  {
   CCandle *node,*lnode;
//--- check
...
//+------------------------------------------------------------------+
void CCandleList::Sort(int mode)
  {
//--- check
   if(m_data_total==0)
      return;
...
//+------------------------------------------------------------------+
bool CCandleList::MoveToIndex(int index)
  {
//--- check
...
//| current one                                                      |
//+------------------------------------------------------------------+
bool CCandleList::Exchange(CCandle *node1,CCandle *node2)
  {
   CCandle *tmp_node,*node;
//--- check
...
//+------------------------------------------------------------------+
CCandle *CCandleList::QuickSearch(CCandle *element)
  {
   int      i,j,m;
   CCandle *t_node=NULL;
//--- check
...
//| Search position of an element in a sorted list                   |
//+------------------------------------------------------------------+
CCandle *CCandleList::Search(CCandle *element)
  {
   CCandle *result;
//--- check
...
//+------------------------------------------------------------------+
bool CCandleList::Save(const int file_handle)
  {
   CCandle *node;
   bool     result=true;
//--- check
...
//+------------------------------------------------------------------+
bool CCandleList::Load(const int file_handle)
  {
   uint     i,num;
   CCandle *node;
   bool     result=true;
